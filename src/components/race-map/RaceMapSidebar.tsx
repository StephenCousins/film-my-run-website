...
export function RaceMapSidebar({
  ...
  return (
    <div className="w-full lg:w-[380px] h-full bg-zinc-900 border-r border-zinc-800 overflow-y-auto flex flex-col">
      {/* Header */}
      <div className="p-6 border-b border-zinc-800 bg-gradient-to-br from-green-500/10 to-transparent">
        <h1 className="text-2xl font-bold text-white mb-1">Race Map</h1>
      ...
      {/* Loading indicator */}
      {loading && (
        <div className="p-5 text-center bg-zinc-950 border-b border-zinc-800">
          <div className="w-10 h-10 border-3 border-zinc-700 border-t-green-500 rounded-full animate-spin mx-auto mb-3" />
          <p className="text-zinc-400 text-sm">Loading routes...</p>
          {progress && (
            <>
        ...
        {filtersExpanded && (
          <div className="mt-4 space-y-3">
            <div className="flex gap-4">
              <label className="flex-1 flex items-center gap-2 p-3 bg-zinc-900 border border-zinc-800 rounded-lg cursor-pointer hover:border-zinc-700 transition-colors">
                <input
                  type="checkbox"
                ...
                />
                <span className="text-sm text-zinc-300">Marathons</span>
              </label>
              <label className="flex-1 flex items-center gap-2 p-3 bg-zinc-900 border border-zinc-800 rounded-lg cursor-pointer hover:border-zinc-700 transition-colors">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 accent-green-500"
                />
                <span className="text-sm text-zinc-300">Ultras</span>
              </label>
            </div>
            ...
            <div className="flex gap-4">
              <label className="flex-1 flex items-center gap-2 p-3 bg-zinc-900 border border-zinc-800 rounded-lg cursor-pointer hover:border-zinc-700 transition-colors">
                <input
                  ...
                  className="w-4 h-4 accent-green-500"
                />
                <span className="text-sm text-zinc-300">Road</span>
              </label>
              <label className="flex-1 flex items-center gap-2 p-3 bg-zinc-900 border border-zinc-800 rounded-lg cursor-pointer hover:border-zinc-700 transition-colors">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 accent-green-500"
                />
                <span className="text-sm text-zinc-300">Trail</span>
              </label>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full p-3 bg-zinc-900 border border-zinc-800 rounded-lg text-sm text-white cursor-pointer hover:border-zinc-700 focus:border-green-500 focus:outline-none transition-colors"
              >
      ...
      {selectedRace && (
        <div className="p-4 border-b border-zinc-800 bg-gradient-to-br from-green-500/5 to-transparent">
          ...
          {detailsExpanded && (
            <div>
              ...
              <div className="bg-zinc-950 rounded-xl p-1 mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selectedRace.video && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-4 py-2.5 bg-zinc-800 text-white font-semibold text-sm rounded-lg hover:bg-zinc-700 transition-colors"
                  >
      ...
      <div className="flex-1 p-4 overflow-y-auto min-h-[300px]">
        <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wide mb-4 sticky top-0 bg-zinc-900 py-2 border-b border-zinc-800 -mt-2 -mx-4 px-4">
          All Races
        </h3>
        <div className="space-y-2">
          {races.map((race) => (
            <button
              ...
              className={cn(
                'w-full p-3.5 bg-zinc-950 border rounded-xl text-left transition-all',
                selectedRace?.number === race.number
                  ? 'border-green-500 border-2 bg-green-500/10'
                  : 'border-zinc-800 hover:border-green-500/50 hover:bg-green-500/5'
              )}
            >
              <div className="flex items-center justify-between mb-2">
                <span className="font-semibold text-white text-sm flex-1 mr-3">{race.name}</span>
                <span
                  className={cn(
