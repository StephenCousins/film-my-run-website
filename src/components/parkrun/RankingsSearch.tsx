    ...
    <div className="space-y-4">
      ...
      {completion && (
        <div className="bg-surface rounded-xl p-4 border border-border">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-muted">UK Parkruns Completed</span>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-3 bg-surface border border-border rounded-xl text-foreground placeholder-muted focus:outline-none focus:border-green-500 transition-colors"
        />
        {search && (
          <button
            onClick={() => setSearch('')}
      ...
      <div className="bg-surface rounded-xl border border-border overflow-hidden">
        {/* Header */}
        <div className="grid grid-cols-12 gap-2 p-3 bg-surface-tertiary/50 text-xs text-muted font-medium">
          ...
          <AnimatePresence>
            {displayedRankings.map((ranking, index) => (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                className={`
                  grid grid-cols-12 gap-2 p-3 items-center border-t border-border
              ...
              >
                ...
                <div className="col-span-5 flex items-center gap-2">
                  ...
                  <span className={`text-sm truncate ${ranking.user_visited ? 'text-foreground' : 'text-muted'}`}>
                    {ranking.venue}
                  </span>
                </div>
                ...
                <div className="col-span-2 text-center">
                  {ranking.user_visits > 0 ? (
                    <span className="text-secondary text-sm">{ranking.user_visits}</span>
                  ) : (
                    <span className="text-muted">-</span>
                  )}
                </div>
          ...
          <div className="p-3 border-t border-border">
            <div className="flex gap-2 justify-center">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-surface-tertiary hover:bg-border rounded-lg text-sm text-secondary transition-colors"
              >
                {isExpanded ? (
                  <>
