...
export function YearCard({ stats, index, cumulativeTotal }: YearCardProps) {
  return (
    <motion.div
      ...
      className="flex-shrink-0 w-72 bg-surface rounded-2xl border border-border overflow-hidden shadow-sm"
    >
      {/* Header */}
      <div className="bg-gradient-to-r from-green-500/20 to-transparent p-4 border-b border-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Calendar className="w-5 h-5 text-green-500" />
            <span className="text-2xl font-bold text-foreground">{stats.year}</span>
          </div>
          <span className="text-xs text-muted bg-surface-tertiary px-2 py-1 rounded-full">
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xl font-bold text-foreground font-mono">{stats.runs}</span>
          </div>
          ...
          <p className="text-xs text-muted mt-1">{Math.round((stats.runs / 52) * 100)}% of Saturdays</p>
        </div>
        ...
        <div className="flex items-center justify-between py-2 border-t border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted">Best Time</span>
          </div>
          <span className="font-mono font-medium text-green-500">{stats.best_time_formatted}</span>
        </div>
        ...
        <div className="flex items-center justify-between py-2 border-t border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted">Best Position</span>
          </div>
          <span className="font-mono font-medium text-foreground">{stats.best_position}</span>
        </div>
        ...
        return (
          ...
          >
            ...
            <div className="relative" style={{ height: `${chartHeight}px`, width: '32px' }}>
              ...
              >
                {/* Tooltip */}
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-surface-tertiary px-2 py-1 rounded text-xs text-foreground opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                  {year.runs} runs
                </div>
              </motion.div>
            </div>
            {/* Year label */}
            <span className="text-xs text-muted mt-2">{year.year.toString().slice(-2)}</span>
          </div>
        );
      })}
    </div>
