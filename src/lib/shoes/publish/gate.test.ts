...
describe('evaluate', () => {
  ...
  it('holds too_old before fetching reviews', async () => {
    ...
    const r = await evaluate(cand(), {
      ...ok(),
      findBrandProductPage: async () => ({ ...page, releaseDate: new Date('2024-01-01') }),
  ...
  it('holds too_old on the earliest review date, not the latest', async () => {
    ...
    const r = await evaluate(cand({ evidence: { sources } }), { ...ok(), findBrandProductPage: async () => ({ ...page, releaseDate: null }) });
    expect(r).toMatchObject({ publish: false, reasons: ['too_old'] });
  });
  it('does not hold on an unknown release date', async () => {
    const r = await evaluate(cand(), { ...ok(), findBrandProductPage: async () => ({ ...page, releaseDate: null }) });
    expect(r.publish).toBe(true);
  ...
  it('builds the specs context from title, product and stripped page text', async () => {
    ...
    const r = await evaluate(cand(), {
      ...ok(),
      findBrandProductPage: async () => ({ ...page, html, product: { name: 'Clifton 10', image: [], description: 'Cushioned daily trainer' } }),
